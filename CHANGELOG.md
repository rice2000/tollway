...
### Added
- `llms.txt` — LLM-friendly project overview following the llmstxt.org standard; covers config reference, payment flow, and links to key source files
- `llms-full.txt` — complete flat-text documentation for LLM consumption; full API reference, all config options, code examples, and architecture overview in one file
- `CLAUDE.md` — Claude Code context for working in this repo, plus a ready-to-paste snippet developers can add to their own project's `CLAUDE.md` when integrating Tollway

## [0.0.3] - 2026-03-30

### Added
- Interactive CLI demo (`demo/index.js`) — starts a real Tollway server, walks through the full x402 payment flow with scripted requests, then drops into a live REPL
- `npm run demo` script at the workspace root
